...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    means.map(y => y -> GenSeq()).toMap ++ points.groupBy(x => findClosest(x, means))
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val classification: GenMap[Point, GenSeq[Point]] = classify(points, means)
    val newmeans: GenSeq[Point] = update(classification, means)
